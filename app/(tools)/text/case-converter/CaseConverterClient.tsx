...
const CASE_OPTIONS: { id: CaseType; label: string; icon: any; hint: string }[] = [
...
export function CaseConverterClient() {
    ...
    return (
        <div className="flex flex-col gap-6">
            {/* Input & Output Area (Single dynamic box) */}
                ...
                {CASE_OPTIONS.map((opt) => {
                    ...
                    return (
                        <button
                            ...
                            disabled={!text}
                            className="group flex flex-col items-center gap-2 rounded-xl border border-border bg-card p-4 transition-all hover:border-blue-500/50 hover:bg-blue-500/5 disabled:cursor-not-allowed disabled:opacity-50"
                        >
            ...
            <div className="rounded-xl border border-border bg-card/30 p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-muted-foreground">
                    <p>• <strong>Sentence case</strong> tự động nhận diện dấu chấm để viết hoa đầu câu mới.</p>
                    <p>• <strong>Snake/Camel case</strong> hỗ trợ dọn dẹp các ký tự đặc biệt để dùng trong lập trình.</p>
                </div>
            </div>
        </div>
    );
